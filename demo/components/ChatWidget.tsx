...
export default function ChatWidget({
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { id: crypto.randomUUID(), role: 'assistant', content: data.ai_response }])
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Something went wrong.')
    } finally {
      setLoading(false)
  ...
  return (
    <>
      ...
      >
        {open ? <X className="h-4 w-4" /> : <><MessageCircle className="h-4 w-4" /><span>{buttonLabel}</span></>}
      </button>
      ...
      {open && (
        <div className="fixed bottom-20 right-6 z-50 flex w-80 flex-col rounded-2xl border border-gray-200 bg-white shadow-2xl sm:w-96">
          <div className="flex items-center justify-between rounded-t-2xl bg-indigo-600 px-4 py-3">
            ...
            <button onClick={() => setOpen(false)} aria-label="Close chat" className="rounded-full p-1 text-indigo-200 hover:bg-indigo-500 hover:text-white">
              <X className="h-4 w-4" />
            </button>
          </div>

          <div className="flex flex-1 flex-col gap-3 overflow-y-auto p-4" style={{ maxHeight: '360px' }}>
            {initializing && <div className="flex items-center gap-2 text-sm text-gray-400"><Loader2 className="h-4 w-4 animate-spin" /><span>Starting conversation…</span></div>}
            {messages.length === 0 && !initializing && <p className="text-sm text-gray-400">Ask me anything to get started.</p>}
          ...
          <div className="border-t border-gray-100 p-3">
            ...
            <p className="mt-1.5 text-center text-[10px] text-gray-300">Built with RAG Kit · Press Enter to send</p>
          </div>
        </div>
      )}
    </>
